...
import application.multiAgent.agentHelper.calScore.CalPoints;
import application.multiAgent.agentHelper.calScore.ComfortablePerson;
import application.multiAgent.agentHelper.calScore.EconomicalPerson;
...
public class ValueCal {
    ...
    public void initPrice(List bids){
        int sumPrice = 0;
        int maxPrice = ((application.multiAgent.ontology.Bid)bids.get(0)).getPrice();
        int minPrice = ((application.multiAgent.ontology.Bid)bids.get(0)).getPrice();
        for(int i=0;i<bids.size();i++){
            int tempPrice = ((application.multiAgent.ontology.Bid)bids.get(i)).getPrice();
            sumPrice+= tempPrice;
    ...
    public List ScreenBids(List bids, Tenant user, Order order, boolean InNegotiation, jade.core.Agent agent){
        ...
        if(user.getPreference().equals("economical")){
            ...
            for(int i=0;i<bids.size();i++){
                application.multiAgent.ontology.Bid tempbid = ((application.multiAgent.ontology.Bid)bids.get(i));
